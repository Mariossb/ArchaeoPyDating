...
class Dating:
    ...
    def pb_h(self, z, h):
        ...
        if tmin[0] > tmax[0]:
            tmax = np.append(tmax, t[-1])
            tmin = np.append(t[0], tmin)
        tmax = int(round(tmax / 5) * 5)
        tmin = int(round(tmin / 5) * 5)
    ...
    def plot(self, zD=None, zI=None, zF=None, z=None,
        ...
        axmapa.gridlines(draw_labels=True, color='black', linestyle='--')
        axmapa.plot(self.dato.lon, self.dato.lat, transform=ccrs.PlateCarree(), marker='o', markersize='14')
        axmapa.plot(self.curva.lon, self.curva.lat, transform=ccrs.PlateCarree(), marker='*', markersize='14')
        if self.dato.lon + 30 > 180:
            axmapa.set_extent([self.dato.lon - 30, 180, self.dato.lat - 20, self.dato.lat + 20], crs=ccrs.PlateCarree())
        elif self.dato.lon - 30 < -180:
            axmapa.set_extent([-180, self.dato.lon + 30, self.dato.lat - 20, self.dato.lat + 20],
